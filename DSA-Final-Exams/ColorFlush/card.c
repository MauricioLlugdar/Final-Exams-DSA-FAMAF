...
int card_drop_points(card fst, card snd, bool isFlush, bool upwardsFlush)
{
    int points = 0;
    if(isFlush){
        if(upwardsFlush){ //Go up
            if(snd->number == fst->number || snd->number == fst->number-1 || snd->number == fst->number+1){
                points = fst->number < snd->number ? 3 : fst->number > snd->number ? 5 : 7;
            } else{
                points = -3;
            }
        }else{ // Go down
        ...
        if(snd->number == fst->number-1 || snd->number == fst->number+1){
            points = 5; // Starting a ladder
        }
    }
